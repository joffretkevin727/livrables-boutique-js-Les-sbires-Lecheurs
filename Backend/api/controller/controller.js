...
const getAllChampions = async (req, res) => {
    try {
        ...
        if (champions.length === 0) {
            return res.status(404).json({ message: 'Aucun champion trouvé' });
        }
...
const getChampionSkins = async (req, res) => {
    try {
        ...
        if (skins.length === 0) {
            return res.status(404).json({ message: 'Aucun skin trouvé' });
        }
...
const getSimilarChampions = async (req, res) => {
    try {
        ...
        if (champions.length === 0) {
            return res.status(404).json({ message: 'Aucun champion similaire trouvé' });
        }
        res.status(200).json(champions);
    } catch (error) {
        res.status(500).json({ message: 'Erreur serveur', error });
...
const sortChampions = async (req, res) => {
    try {
        ...
        if (!order || (order !== 'asc' && order !== 'desc')) {
            return res.status(400).json({ message: 'Order invalide, utilisez asc ou desc' });
        }
...
const deleteFromPanier = async (req, res) => {
    try {
        ...
        res.status(200).json({ message: 'Supprimé du panier' });
    } catch (error) {
        res.status(500).json({ message: 'Erreur serveur', error });
    }
};
...
const createCommande = async (req, res) => {
    try {
        ...
        res.status(201).json({ message: 'Commande créée', id: commande.insertId });
    } catch (error) {
        res.status(500).json({ message: 'Erreur serveur', error });
    }
};
...
const getAdresses = async (req, res) => {
    try {
        ...
        if (adresses.length === 0) {
            return res.status(404).json({ message: 'Aucune adresse trouvée' });
        }
